...
def _sorted_labels(m: nx.Graph) -> nx.Graph:
...
def _permute_molecule(m):
    ...
    # nx's relabel_nodes function does not change the iteration order of the graph's internal node dictionary, which
    # reflects the initial insert order of nodes. _sorted_labels() rebuilds a Graph object from scratch with a node
    # iteration order identical to the label order.
